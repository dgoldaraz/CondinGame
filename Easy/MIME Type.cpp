...
int main()
{
    ...
    map<string, string> extToMime;
    for (int i = 0; i < N; i++) {
        string EXT; // file extension
        string MT; // MIME type.
        cin >> EXT >> MT; cin.ignore();
        std::transform(EXT.begin(), EXT.end(), EXT.begin(), ::tolower);
        extToMime.insert(pair<string,string>(EXT,MT));
    }
    ...
    for (int i = 0; i < Q; i++) {
        ...
        {
            string extension = FNAME.substr(found + 1);
            std::transform(extension.begin(), extension.end(), extension.begin(), ::tolower);
            map<string,string>::iterator it = extToMime.find(extension);
            if(it != extToMime.end())
            {
