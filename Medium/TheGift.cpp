...
int main()
{
    ...
    else
    {
        ...
        cerr << "People : " << values.size() << endl;
        float cC = C;
        float equalValue = C / N;
        float count = 0;
        for(int i = 0; i < N - 1; ++i)
        {
            if( values[i] > equalValue )
            {
                cout << (int)equalValue << endl;
                //We need to assure that the error it's been counting all the time
                count += (int)equalValue;
                cC = cC - (int)equalValue;
                equalValue = cC / (N - (i +1));
            }
            else
            {
