...
void topMovement( int roomType, int& X, int& Y)
{
    //only deal with cases that can happen, return -1 in other case
    switch(roomType)
    {
        case 1:
        case 3:
...
void leftMovement(  int roomType, int& X, int& Y)
{
...
void rightMovement(  int roomType, int& X, int& Y)
{
    switch(roomType)
    {
        case 1: 
...
int main()
{
    ...
    for (int i = 0; i < H; i++) {
        ...
        getline(cin, LINE);
        vector<int> line = splitString(LINE, " ");
        for(int j = 0;j < W; j++)
        {
            grid[i][j] = line[j];
        }
    ...
    while (1) {
        ...
        cerr << "Pos: " << XI << "," << YI << " Dir:" << POS << endl;

        // Write an action using cout. DON'T FORGET THE "<< endl"
        // To debug: cerr << "Debug messages..." << endl;
        
        int roomType = grid[YI][XI];
        cerr << "Room: "<< roomType << endl;
        ...
        if(POS == "TOP")
        {
            topMovement(roomType,XI,YI);
        }
        if(POS == "LEFT")
        {
            leftMovement(roomType,XI,YI);
        }
        if(POS == "RIGHT")
        {
            rightMovement(roomType, XI, YI);
        }
